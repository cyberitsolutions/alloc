...
class wiki_module extends module
{
    ...
    public function file_save($file, $body)
    {
        if (is_dir(dirname($file)) && path_under_path(dirname($file), self::get_wiki_path())) {
            // Save the file ...
      ...
      $index = Zend_Search_Lucene::open(ATTACHMENTS_DIR.'search/wiki');
            $f = str_replace(self::get_wiki_path(), '', $file);
            $hits = $index->find('id:'.$f);
            foreach ($hits as $hit) {
                $index->delete($hit->id);
            }
            self::update_search_index_doc($index, $f);
            $index->commit();
        }
    }
    ...
    public function nuke_trailing_spaces_from_all_lines($str)
    {
    ...
    public function get_file($file, $rev = '')
    {
        global $TPL;

        $f = realpath(self::get_wiki_path().$file);

        if (path_under_path(dirname($f), self::get_wiki_path())) {
            $mt = get_mimetype($f);
      ...
      if ($vcs) {
          $vcs_file = $vcs->cat($f, $rev);
            ...
            if ($vcs && self::nuke_trailing_spaces_from_all_lines($disk_file) != self::nuke_trailing_spaces_from_all_lines($vcs_file)) {
                if (!$vcs_file) {
    ...
    public function update_search_index_doc(&$index, $file)
    {
        // Attempt to parse pdfs
    if (strtolower(substr($file, -4)) == '.pdf') {
        $pdfstr = file_get_contents(self::get_wiki_path().$file);
        $pdf_reader = new pdf_reader();
        $pdfstr = $pdf_reader->pdf2txt($pdfstr);

    // Else regular text
    } else {
        $str = file_get_contents(self::get_wiki_path().$file);
    }

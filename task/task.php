...
if ($_POST["save"] || $_POST["save_and_back"] || $_POST["save_and_new"] || $_POST["save_and_summary"] || $_POST["timeSheet_save"] || $_POST["close_task"]) {
    ...
    if ($success) {
        ...
        // A task can only have a pending task or pending reopen date -
        // pending task is fixed up in JS, but check here too
        ...
        // This is only valid on pending_, but not on pending_task (because it
...
// If we've been sent here by a "New Message" or "New Fault" option in the
// Quick List dropdown
...
if (in_str("pending_", $task->get_value("taskStatus"))) {
    ...
    foreach ($rows as $r) {
        ...
        // Which date gets plugged in is arbitrary, but it would be unusual

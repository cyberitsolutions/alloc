...
class task extends db_entity
{
    ...
    public function get_parent_task_select($projectID = '')
    {
        ...
        $db = new db_alloc();
        if ($projectID) {
            list($ts_open, $ts_pending, $ts_closed) = self::get_task_status_in_set_sql();
    ...
    // Project dropdown
    $TPL['projectOptions'] = self::get_project_options($projectID);
    ...
    $TPL['parentTaskOptions'] = $this->get_parent_task_select();
        ...
        $TPL['task_taskStatusOptions'] = page::select_options(self::get_task_statii_array(true), $this->get_value('taskStatus'));
    ...
    public function get_task_link($_FORM = array())
    {
    ...
    $taskStatii = self::get_task_statii();
        if ($flat) {
            $m = new meta('taskStatus');
            $taskStatii = $m->get_assoc_array();
            foreach ($taskStatii as $status => $arr) {
                $taskStatiiArray[$status] = self::get_task_status_thing('label', $status);
            }
        } else {
    ...
    $filter['taskStatus'] and $sql[] = self::get_taskStatus_sql($filter['taskStatus']);
    ...
    public function get_recursive_child_tasks($taskID_of_parent, $rows = array(), $padding = 0)
    {
        $rtn = array();
        $rows or $rows = array();
        foreach ($rows as $taskID => $v) {
            ...
            if ($taskID_of_parent == $parentTaskID) {
                ...
                unset($rows[$taskID]);
                $padding += 1;
                $children = self::get_recursive_child_tasks($taskID, $rows, $padding);
                $padding -= 1;
    ...
    public function build_recursive_task_list($t = array(), $_FORM = array())
    {
      ...
      $tasks += array($row['taskID'] => $row);
            ...
            if ($r['children']) {
                list($t, $d) = self::build_recursive_task_list($r['children'], $_FORM);
                $t and $tasks += $t;
                $d and $done += $d;
            }
        }
    ...
    list($filter, $having) = self::get_list_filter($_FORM);
    ...
    if (is_array($filter) && count($filter)) {
       ...
       LEFT JOIN tag seltag ON seltag.taskID = task.taskID
        ...
        if ($_FORM['taskView'] == 'byProject') {
            ...
            $t = self::get_recursive_child_tasks($parentTaskID, (array) $rows);
            list($tasks, $done) = self::build_recursive_task_list($t, $_FORM);
    ...
    public function get_list_html($tasks = array(), $ops = array())
    {
        global $TPL;
    ...
    {
        $taskStatii = self::get_task_statii_array(true);
        foreach ($taskStatii as $k => $v) {
            $taskStatiiStr .= $pipe.$k;
            $pipe = ' | ';
        }

        return array('taskView' => '[MANDATORY] eg: byProject | prioritised', 'return' => '[MANDATORY] eg: html | array', 'limit' => 'Appends an SQL limit (only for prioritised and objects views)', 'projectIDs' => 'An array of projectIDs', 'projectID' => 'A single projectID', 'taskStatus' => $taskStatiiStr, 'taskDate' => 'new | due_today | overdue | d_created | d_assigned | d_targetStart | d_targetCompletion | d_actualStart | d_actualCompletion (all the d_* options require a dateOne (From Date) or a dateTwo (To Date) to be filled)', 'dateOne' => 'From Date (must be used with a d_* taskDate option)', 'dateTwo' => 'To Date (must be used with a d_* taskDate option)', 'taskTimeSheetStatus' => 'my_open | not_assigned | my_closed | my_recently_closed | all', 'taskTypeID' => 'Task | Parent | Message | Fault | Milestone', 'current_user' => 'Lets us fake a current_user id for when generating task emails and there is no $current_user object', 'taskID' => 'Task ID', 'starred' => 'Tasks that you have starred', 'taskName' => 'Task Name (eg: *install*)', 'tags' => 'Task tags', 'creatorID' => 'Task creator', 'managerID' => 'The person managing task', 'personID' => 'The person assigned to the task', 'parentTaskID' => 'ID of parent task, all top level tasks have parentTaskID of 0, so this defaults to 0', 'projectType' => 'mine | pm | tsm | pmORtsm | Current | Potential | Archived | all', 'applyFilter' => 'Saves this filter as the persons preference', 'padding' => 'Initial indentation level (useful for byProject lists)', 'url_form_action' => 'The submit action for the filter form', 'hide_field_options' => "Hide the filter's field's panel.", 'form_name' => 'The name of this form, i.e. a handle for referring to this saved form', 'dontSave' => 'Specify that the filter preferences should not be saved this time', 'skipObject' => 'Services coming over SOAP should set this true to minimize the amount of bandwidth', 'showDates' => 'Show dates 1-4', 'showDate1' => 'Date Target Start', 'showDate2' => 'Date Target Completion', 'showDate3' => 'Date Actual Start', 'showDate4' => 'Date Actual Completion', 'showDate5' => 'Date Created', 'showProject' => 'The tasks Project (has different layout when prioritised vs byProject)', 'showPriority' => 'The calculated overall priority, then the tasks, then the projects priority', 'showPriorityFactor' => 'The calculated overall priority', 'showStatus' => 'A colour coded textual description of the status of the task', 'showCreator' => 'The tasks creator', 'showAssigned' => 'The person assigned to the task', 'showTimes' => 'The original estimate and the time billed and percentage', 'showHeader' => 'A descriptive html header row', 'showDescription' => 'The tasks description', 'showComments' => 'The tasks comments', 'showTaskID' => 'The task ID', 'showParentID' => "The task's parent ID", 'showManager' => 'Show the tasks manager', 'showPercent' => 'The percent complete', 'showTags' => "The task's tags", 'showEdit' => 'Display the html edit controls to allow en masse task editing', 'showTotals' => 'Display the totals of certain columns in the html view',
                );
    }
    ...
    public static function load_form_data($defaults = array())
    {
        ...
        $page_vars = array_keys(self::get_list_vars());
    ...
    has('project') and $rtn['projectOptions'] = project::get_list_dropdown($_FORM['projectType'], $_FORM['projectID']);
        ...
        $taskStatii = self::get_task_statii_array();
    ...
    $rows = array();
        ...
        break;
        case 'projectID':
          self::load_entity('project', $newValue, $newProject);
        ...
        break;
        case 'parentTaskID':
          self::load_entity('task', $newValue, $newTask);
          if ($newValue) {
        ...
        break;
        case 'taskStatus':
          $changeDescription = sprintf('Task status set to <span style="background-color:%s">%s</span>.', self::get_task_status_thing('colour', $newValue), self::get_task_status_thing('label', $newValue)
                                      );
        break;

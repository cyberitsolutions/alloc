...
{
    // each ip can be either a username or an email address. If it's a
    // username, look them up. If it's an email address, see if it belongs to
    // anybody. If the email doesn't match a client contact, drop it - if it's
    // added as an IP their name can't be changed later.

    foreach ($parties as $party) {
        $ipdata = array('entity' => 'task', 'entityID' => $taskID);
        // same logic as the real interested parties code - if the name
        // contains an '@', it's an email address. Otherwise, it's a login.
...
function import_gnome_planner($infile)
{
    ...
    if (is_uploaded_file($filename)) {
        ...
        // This function does two passes of each file. First, it does a set of
        // basic tests to check the file is valid. If it is, the function then
        // actually imports the data.
        $result[] = "<li>Checking the file for validity...</li>";
        // Check that every <resource /> element has a short-name that
        // corresponds to the user id of a user we know about
...
function import_planner_tasks($parentNode, $parentTaskId, $depth, $task_allocation, $resource_people, $project_manager_ID)
{
    ...
    for ($i = 0; $i < $parentNode->childNodes->length; $i++) {
        ...
        if ($taskXML->nodeType == XML_ELEMENT_NODE && $taskXML->tagName == "task") {
            ...
            $task->set_value('projectID', $projectID);
            // We can find the task assignee's id in the $task_allocation array,
            // and that person's Person record in the $resource_people array
            ...
            // The following fields we leave at their default values:
            // duplicateTaskID, dateActualCompletion, dateActualStart, closerID,
            // timeExpected, dateClosed, parentTaskID, taskModifiedUser

            // Handle task assignment
            if (isset($task_allocation[$planner_taskid])) {
                if (is_array($task_allocation[$planner_taskid])) {
                    // This task was assigned to more than one person. Assign
                    // it to the project manager and make a comment about it.
...
function export_gnome_planner($projectID)
{
    ...
    // We need to sort by taskID (we assume taskIDs were assigned linearly on
    // import) otherwise Planner will get very confused with ordering
    ...
    foreach ($tasks as $task) {
        ...
        if (!$task["dateActualCompletion"]) {
            if (!$task["dateTargetCompletion"]) {
                // The task has to last for some amount of time, so end = start
                // (otherwise we get end = 1970)
        ...
        // That's the total number of seconds, Planner expects the number of
        // 8-hour days worth of seconds
...
function export_csv($projectID)
{
    ...
    // Sort by taskID, we assume taskIDs were assigned linearly on import/creation,
    // so as to produce an identical file
    foreach ($tasks as $task) {
        $taskIDs[] = $task['taskID'];
    }
    array_multisort($taskIDs, $tasks);
...
{
    // Takes a GNOME Planner formatted date (YYYYMMDDTHHMMSSZ, where "T" and
    // "Z" are literal) and converts it into a MySQL formatted date (Y-m-d H:i:s)
...
{
    // Attempts to find a Person record that corresponds to one of the names
    // specified in the $candidates array. Returns false on failure.
    global $db;
...
class DOM_XML_Wrapper_Document
{
    ...
    public function createElement($element_name)
    {
...
{
    // Technically this class implements the functionality of both DOMNode and
    // DOMElement. For simplicity of the wrapper we have the function as part
    // of one object
    public function DOM_XML_Wrapper_Node($original_node)
    {
...
{
    public function DOM_XML_Wrapper_NodeList($in_array)
    {

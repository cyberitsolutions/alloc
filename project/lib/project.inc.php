...
class project extends db_entity
{
    ...
    public function save()
    {
        ...
        } elseif ($old["projectStatus"] == "Archived" && $this->get_value("projectStatus") != "Archived") {
    ...
    public function get_project_link($_FORM = array())
    {
        global $TPL;
    ...
    public function has_project_permission($person = "", $permissions = array())
    {
    ...
    public function get_project_people_by_role($role = "")
    {
        $rows = array();
    ...
    public function get_navigation_links($ops = array())
    {
        global $taskID;
        global $TPL;
        $current_user = &singleton("current_user");
        ...
        if ($type == "mine") {
            $q = prepare("SELECT project.projectID, project.projectName
                       ...
                       LEFT JOIN role ON projectPerson.roleID = role.roleID
                        ...
                        GROUP BY projectID
                        ORDER BY project.projectName", $personID);
        } elseif ($type == "pm") {
            $q = prepare("SELECT project.projectID, project.projectName
                            FROM project
                       LEFT JOIN projectPerson ON project.projectID = projectPerson.projectID
    ...
    public function get_list_dropdown($type = "mine", $projectIDs = array())
    {
    ...
    public function get_list_dropdown_options($type = "mine", $projectIDs = array(), $maxlength = 35)
    {
        $db = new db_alloc();
    ...
    {
        // This is the definitive method of getting a list of projects that
        ...
        $q = "SELECT project.*, client.*
                FROM project".$from."
           LEFT JOIN client ON project.clientID = client.clientID".$filter."
    ...
    public function get_prepaid_invoice()
    {
        ...
        } elseif ($this->get_value("clientID")) {
            $q = prepare("SELECT *
                            FROM invoice
                           WHERE clientID = %d
    ...
    public static function get_projectID_sql($filter, $table = "project")
    {
        if (!$filter["projectID"] && $filter["projectType"] && $filter["projectType"] != "all") {
            ...
            // Oi! What a pickle. Need this flag for when someone doesn't have
            // entries loaded in the above while loop.
            $firstOption = true;
        ...
        // If passed array projectIDs then join them up with commars and put
        // them in an sql subset
        if (is_array($filter["projectIDs"]) && count($filter["projectIDs"])) {
            return sprintf_implode("(".$table.".projectID = %d)", $filter["projectIDs"]);

        // If there are no projects in $filter["projectIDs"][] and we're

...
class stats
{
    ...
    {
        // date from which a project is counted as being new. if monday then
        // date back to friday, else the previous day
        $days = date("w") == 1 ? 3 : 1;
    ...
    public function task_stats()
    {
        ...
        // date from which a task is counted as being new. if monday then date
        // back to friday, else the previous day
        $days = date("w") == 1 ? 3 : 1;
        $date = date("Y-m-d", mktime(0, 0, 0, date("m"), date("d") - $days, date("Y")));
        // Get total amount of completed tasks for every person
    ...
    {
        // date from which a comment is counted as being new. if monday then
        // date back to friday, else the previous day
        $days = date("w") == 1 ? 3 : 1;
        $date = date("Y-m-d", mktime(0, 0, 0, date("m"), date("d") - $days, date("Y")));

    <tr class="{odd_even}">
      <td align="left"><a href="{user_graph_big}">{user_username}</a></td>
      <td align="center">{user_projects_current}</td>
      <td align="center">{user_projects_total}</td>
      <td align="center">{user_tasks_current}</td>
      <td align="center">{user_tasks_total}</td>
      <td align="center">{user_comments_total}</td>
      <td align="center" valign="bottom"><a href="{user_graph_big}"><img src="{user_graph}" border=0></a></td>
    </tr>

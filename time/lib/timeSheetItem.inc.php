...
class timeSheetItem extends db_entity
{
    ...
    {
        // This is the definitive method of getting a list of timeSheetItems
        ...
        $q = prepare("SELECT dateTimeSheetItem, sum(timeSheetItemDuration*timeUnitSeconds) / 3600 AS hours
                   ...
                   LEFT JOIN timeUnit ON timeUnitID = timeSheetItemDurationUnitID
                       WHERE personID = %d
                         ...
                         AND dateTimeSheetItem <= '%s'
                    GROUP BY dateTimeSheetItem", $personID, $start, $end);
        $db = new db_alloc();
        $db->query($q);
        while ($row = $db->row()) {
        ...
        $q = prepare("SELECT CONCAT(YEAR(dateTimeSheetItem),'-',MONTH(dateTimeSheetItem)) AS dateTimeSheetItem
               ...
               LEFT JOIN timeUnit ON timeUnitID = timeSheetItemDurationUnitID
                   WHERE personID = %d
                     AND dateTimeSheetItem >= '%s'
                     AND dateTimeSheetItem <= '%s'
                GROUP BY YEAR(dateTimeSheetItem), MONTH(dateTimeSheetItem)", $personID, $start, $end);
        $db = new db_alloc();
        $db->query($q);
        while ($row = $db->row()) {
            $f = explode("-", $row["dateTimeSheetItem"]);
            // the %02d is just to make sure the months are consistently zero padded
            $info[sprintf("%4d-%02d", $f[0], $f[1])] = $row;
        }

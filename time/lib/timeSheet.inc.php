...
class timeSheet extends db_entity
{
    ...
    {
        // NOTE:: what the hell is this here for? -- cjbayliss, 2018-11
        /***************************************************************************
         *                                                                         *
        ...
        $db->query(prepare("SELECT SUM(timeSheetItemDuration) AS total_duration,
                         ...
                         LEFT JOIN currencyType on currencyType.currencyTypeID = timeSheet.currencyTypeID
                             WHERE timeSheetItem.timeSheetID = %d", $this->get_id()));
    ...
    public function createTransactions($status = "pending")
    {
        ...
        } elseif ($this->get_value("recipient_tfID") == "") {
        ...
        } elseif (!$cost_centre || $cost_centre == 0) {
        ...
        // 2. Payment Insurance
        // NOTE:: what is removed? need more clarity in comments -- cjbayliss, 2018-11
        // removed
            ...
            */
            ...
            while ($db->next_record()) {
                ...
                } elseif ($db->f("commissionPercent") == 0) {
    ...
    {
        // This is the definitive method of getting a list of timeSheets that
    ...
    public function get_list_html($rows = array(), $extra = array())
    {
        global $TPL;
    ...
    {
        // access controls are partially disabled for timesheets. Make sure
        // time sheet is really accessible by checking user ID - it's
        // restricted to being NOT NULL in the DB. Not doing this check allows
        // a user to overwrite an existing timesheet with a new one assigned
        // to themself.
    ...
    {
        // is possible to move backwards to "edit", from both "manager" and
        // "admin" requires manager or APPROVE_TIMESHEET permission
        $current_user = &singleton("current_user");
        $project = $this->get_foreign_object("project");
...
EOD;
    ...
    public function email_move_status_to_manager($direction, $info)
    {
        ...
        if ($direction == "forwards") {
            // forward to manager requires the timesheet to be owned by the
            // current user or TIME_INVOICE_TIMESHEETS project managers may
            // not do this
...
EOD;
    ...
    public function email_move_status_to_admin($direction, $info)
    {
        ...
        if ($direction == "forwards") {
            // 3 ways to have permission to do this project manager for the
            // timesheet no project manager and owner of the timesheet the
            // permission flag
            if (!(in_array($current_user->get_id(), $projectManagers) ||
...
EOD;
    ...
    {
        $current_user = &singleton("current_user");
        // Can get forwards to "invoiced" from "admin" requires
        // INVOICE_TIMESHEETS
        if (!$this->have_perm(PERM_TIME_INVOICE_TIMESHEETS)) {
            //no permission, go away
            alloc_error("You do not have permission to change this timesheet.");

...
function show_timeSheet_list($template)
{
    ...
    while ($db->next_record()) {
        ...
        // Highlight the rate if the project person has a non-zero rate and it
        // doesn't match the item's rate
        if ($default_rate) {
...
|| $_POST["save_and_MoveBack"]) {
    ...
    if ($TPL['message'] || $save_error) {
        ...
        // if TS ID is set but person ID is not, it's an existing timesheet
        // this user doesn't have access to (and will overwrite). Don't
        // proceed.
        $url = $TPL["url_alloc_timeSheet"];
...
// Take care of the transaction line items on an invoiced timesheet created by
// admin
...
if ($timeSheet->get_value("status") == "edit") {
    ...
    if ($preferred_tfID = $tf_db->f("preferred_tfID")) {
        ...
        // The person has a preferred TF, and is a tfPerson for it too
        if ($tf_db->next_record()) {

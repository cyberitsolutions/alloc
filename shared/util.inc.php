...
{
    // I am crippling this function to make its purpose clearer, max 6
    // arguments.
    //
    ...
    //
    // We default to joining by OR, but if the first argument passed doesn't
    // contain a percentage (ie the sprintf marker), then we assume the first
    // arg is the glue, and we bump all the args along one. This changes the
    // usage of the function to:
    //
    ...
    $x = 0;
    while ($x < $length) {
        $rtn.= $comma.sprintf($str, db_esc($f["arg1"][$x]), db_esc($f["arg2"][$x]), db_esc($f["arg3"][$x]), db_esc($f["arg4"][$x]), db_esc($f["arg5"][$x]), db_esc($f["arg6"][$x]));
        $comma = $glue;
        $x++;
    }
    return "(".$rtn.")";

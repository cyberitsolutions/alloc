...
## We propagate session IDs with cookies
  var $fallback_mode;
## If this doesn't work, fall back...
  var $lifetime = 0;
...
## Set this to 'private' to allow private
  ...
  function get_id($id = "") {
    ...
    case "get":
      if (isset($QUERY_STRING)) {
        $QUERY_STRING = ereg_replace("(^|&)".quotemeta(urlencode($this->name))."=".$id."(&|$)", "\\1", $QUERY_STRING);
      }
      break;
    default:
      ;
...
## session $id.
  ...
  function self_url() {
    ...
    return $this->url($PHP_SELF.((isset($QUERY_STRING) && ("" != $QUERY_STRING)) ? "?".$QUERY_STRING : ""));
  }
...
##
# Edited by alla @ Tue Jul 27 15:47:31 EST 2004
# This function seems to exist twice sooo.. let's comment this one out
#  function gc() {

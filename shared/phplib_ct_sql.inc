...
## end of configuration
  ...
  function ac_gc($gc_time, $name) {
    ...
    $this->db->query(sprintf("DELETE FROM %s WHERE changed < '%s' AND name = '%s'", $this->database_table, $sqldate, addslashes($name)));
  }
...
## update duration of visit
    ...
    $squery = sprintf("select count(*) as record_exists from %s where sid='%s' and name='%s'", $this->database_table, $id, $name);
    $uquery = sprintf("update %s set val='%s', changed='%s' where sid='%s' and name='%s'", $this->database_table, $str, $now, $id, $name);
    $iquery = sprintf("insert into %s ( sid, name, val, changed ) values ('%s', '%s', '%s', '%s')", $this->database_table, $id, $name, $str, $now);

    $this->db->query($squery);
    $this->db->next_record();
    if ($this->db->f("record_exists")) {
      $this->db->query($uquery);
      $ret = true;
    } else {
      $ret = $this->db->query($iquery);
    }
    return $ret;
  }

  function ac_delete($id, $name) {
    $this->db->query(sprintf("delete from %s where name = '%s' and sid = '%s'", $this->database_table, addslashes($name), $id));
  }

  function ac_get_value($id, $name) {
    $this->db->query(sprintf("select val from %s where sid  = '%s' and name = '%s'", $this->database_table, $id, addslashes($name)));

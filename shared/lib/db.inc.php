...
class db
{
    ...
    public function save($table, $row = array(), $debug = 0)
    {
        ...
        if ($do_update) {
            $q = sprintf("UPDATE %s SET %s WHERE %s", $table, $this->get_update_str($row), $this->get_update_str($keys, " AND "));
            ...
            $q = sprintf("INSERT INTO %s (%s) VALUES (%s)", $table, $this->get_insert_str_fields($row), $this->get_insert_str_values($row));
    ...
    public function delete($table, $row = array(), $debug = 0)
    {
        $row = $this->unset_invalid_field_names($table, $row);
        $q = sprintf("DELETE FROM %s WHERE %s", $table, $this->get_update_str($row, " AND "));
    ...
    public function unset_invalid_field_names($table, $row, $keys = array())
    {

...
class db_entity
{
    ...
    public function have_perm($action = 0, $person = "", $assume_owner = false)
    {
        ...
        } elseif (isset($permission_cache[$record_cache_key])) {
    ...
    public function perm_cleanup(&$row = array())
    {
    ...
    public function delete()
    {
        if (!$this->have_perm(PERM_DELETE)) {
            $current_user = &singleton("current_user");
            alloc_error(sprintf("Person %d does not have permission %s for %s #%d", $current_user->get_id(), $this->permissions[PERM_DELETE], $this->data_table, $this->get_id()));
            return false;
        }
        if (!$this->has_key_values()) {
            return false;
    ...
    public function insert()
    {
        ...
        if (!$this->have_perm(PERM_CREATE)) {
            ...
            alloc_error(sprintf("Person %d does not have permission %s for %s #%d", $current_user_id, $this->permissions[PERM_CREATE], $this->data_table, $this_id));
            return false;
        }
    ...
    public function update()
    {
        if (!$this->have_perm(PERM_UPDATE)) {
            $current_user = &singleton("current_user");
            alloc_error(sprintf("Person %d does not have permission %s for %s #%d", $current_user->get_id(), $this->permissions[PERM_UPDATE], $this->data_table, $this->get_id()));
            return false;
        }
    ...
    public function is_owner($person = "")
    {
        ...
        if (is_object($person) && $person->classname == "person") {
            ...
            } elseif ($this->key_field->get_name() == "personID") {
    ...
    public function get_assoc_array($key = false, $value = false, $sel = false, $where = array())
    {

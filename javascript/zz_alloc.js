...
function refreshProjectList(radiobutton) {
  url =
    get_alloc_var("url") +
    "task/updateProjectList.php?projectType=" +
    radiobutton.value;
...
function makeAjaxRequest(url, entityid, extra_fields, redraw) {
  $("#" + entityid).html(
    '<img class="ticker" src="../images/ticker2.gif" alt="Updating field..." title="Updating field...">'
  );
...
function help_text_on(id, str) {
  $("#main").append(
    "<div id='helper' class='corner' style='display:none'></div>"
  );
...
$(document).ready(function () {
  ...
  $("table.sortable").each(function () {
    ...
    var arr = $(
      "<i class='arrow icon-circle-arrow-up' style='font-size:95%'></i>"
    ...
    t.bind("aftertablesort", function (event, data) {
      ...
      th.eq(data.column)
        .find(".arrow")
        .removeClass("icon-circle-arrow-down icon-circle-arrow-up")
        .addClass(
          data.direction === "asc"
  ...
  $("input.datefield").bind("dblclick", function (e) {
    ...
    this.value =
      now.getFullYear() +
      "-" +
      (1e2 + now.getMonth() + 1 + "").substr(1) +
      "-" +
      (1e2 + now.getDate() + "").substr(1);
  });
  ...
  $(".commentreply").click(function (e) {
    ...
    $("#id_new_comment")
      ...
      .val($(this).parents(".pcomment").attr("data-comment-id"));
    return false;
  });
  ...
  $(".commentnew").click(function (e) {
    ...
    $("#id_new_comment")
      .find("input[name=entity]")
      .val($("#id_new_comment").find("input[name=commentMaster]").val());
    $("#id_new_comment")
      .find("input[name=entityID]")
      .val($("#id_new_comment").find("input[name=commentMasterID]").val());
    $("#new_comment_container").append(
      $("#id_new_comment").css({
        display: "inline-table",
      })
    );

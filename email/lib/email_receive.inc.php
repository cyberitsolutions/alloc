...
class email_receive
{
    ...
    {
        if ($this->msg_uid) {
            // EDIT by cjbayliss 2018-11 -- if it doesn't work then why is it
            // here? like wtf?
            // this doesn't work!
            imap_setflag_full($this->connection, $this->msg_uid, "\\SEEN", FT_UID);
            // this seems to force it to be marked seen
    ...
    public function add_part_to_array($struct, $partno)
    {
        ...
        if ($struct->type == 2) {
            if (sizeof($struct->parts) > 0) {
                foreach ($struct->parts as $count => $part) {
                    // Iterate here again to compensate for the broken way
                    // that imap_fetchbody() handles attachments
    ...
    public function get_converted_encoding()
    {
        ...
        // if the email has a different encoding, change it to the DB
        // connection encoding so mysql doesn't choke

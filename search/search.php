...
if ($search && $needle && $category == "search_projects") {
    ...
    if (!$noRedirect && is_numeric($needle)) {
        ...
        foreach ($hits as $hit) {
            ...
            $row["title"] = $d->getFieldValue('id')." ".sprintf("<a href='%sprojectID=%d'>%s</a>", $TPL["url_alloc_project"], $d->getFieldValue('id'), page::htmlentities($d->getFieldValue('name')));
            $row["related"] = sprintf("<a href='%sclientID=%d'>%s</a>", $TPL["url_alloc_client"], $d->getFieldValue('cid'), page::htmlentities($d->getFieldValue('client')));
...
// Tasks Search
    ...
    if (!$noRedirect && is_numeric($needle)) {
        ...
        foreach ($hits as $hit) {
            ...
            $row["idx"] = $hit->id;
            $row["score"] = sprintf('%d%%', $hit->score*100);
            $row["title"] = $d->getFieldValue('id')." ".sprintf("<a href='%staskID=%d'>%s</a>", $TPL["url_alloc_task"], $d->getFieldValue('id'), page::htmlentities($d->getFieldValue('name')));
...
// Wiki Search
} elseif ($search && $needle && $category == "search_wiki") {

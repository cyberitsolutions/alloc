    ...
    if (is_object($invoice) && $invoice->get_value("invoiceStatus") == 'edit' && $current_user->have_role('admin')) {
        ...
        if ($invoice->get_value("clientID")) {
            ...
            $q = prepare("SELECT *
                            FROM productSale
                           WHERE clientID = %d
                             AND status = 'admin'", $invoice->get_value("clientID"));
...
if ($current_user->have_role('admin')) {
    if (!$invoiceID) {
        ...
        $TPL["invoice_buttons"] = '<button type="submit" name="save" value="1" class="save_button">Create Invoice<i class="icon-ok-sign"></i></button>';

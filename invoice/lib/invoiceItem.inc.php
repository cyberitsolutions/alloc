...
class invoiceItem extends db_entity
{
    ...
    public function close_related_entity()
    {
        ...
        // It checks for approved transactions and only approves the
        // timesheets or expenseforms that are completely paid for by an
        // invoice item.
        $db = new db_alloc();
        ...
        if ($timeSheetID) {
            ...
            if ($timeSheet->get_value("status") == "invoiced") {
                // If the time sheet doesn't have any transactions and it is
                // in status invoiced, then we'll simulate the "Create Default
                // Transactions" button being pressed.
    ...
    public function create_transaction($amount, $tfID, $status)
    {
        ...
        // If there already a transaction for this invoiceItem, use it instead
        ...
        // If there already a transaction for this timeSheet, use it instead of
        // creating a new one
        if ($this->get_value("timeSheetID")) {
            $q = prepare("SELECT *
                            FROM transaction
